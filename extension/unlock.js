 ...
 * Skip install while a CF challenge is active; allow reload/navigation during challenge.
 */
(function devtoolsUnlock() {
  ...
  /** CF interstitial title patterns (multi-locale). */
  const CF_TITLE_RE =
    /just a moment|请稍候|checking your browser|um momento|un momento|bitte warten|veuillez patienter|대기|少々お待ち|ожидание/i;

  /** Whether this document looks like an active Cloudflare challenge page. */
  function isCloudflareChallengePage() {
    try {
      if (typeof window._cf_chl_opt === 'object' && window._cf_chl_opt) {
        return true;
      }
      if (CF_TITLE_RE.test(document.title || '')) {
        return true;
      }
      ...
      if (
        ...
        document.querySelector(
          '.cf-browser-verification, #cf-challenge-running, [name="cf-turnstile-response"]'
        )
      ) {
        return true;
      }
  ...
  /**
   * Boot: never hook during CF challenge (page usually full-reloads after pass).
   * On normal pages install immediately so anti-devtools cannot win the race.
   */
  function boot() {
    if (isCloudflareChallengePage()) {
      console.warn(TAG, 'Cloudflare challenge detected — deferring unlock hooks');
      waitForChallengeEndThenInstall();
      return;
    }
    installUnlock();
  }

  /** Poll until CF interstitial is gone, then install (or give up after timeout). */
  function waitForChallengeEndThenInstall() {
    const started = Date.now();
    /** Max wait for challenge UI to clear without a full navigation. */
    const MAX_WAIT_MS = 120000;
    const timer = setInterval(function unlockCfWaitTick() {
      if (!isCloudflareChallengePage()) {
        clearInterval(timer);
        installUnlock();
        return;
      ...
      if (Date.now() - started > MAX_WAIT_MS) {
        clearInterval(timer);
        console.warn(TAG, 'CF challenge still present after timeout — installing anyway');
        installUnlock();
      }
    }, 300);
  }
  ...
  /**
   * Decide when to boot. Prefer earliest install on normal pages.
   * Only wait for head markers when the document is still empty (true document_start).
   * Do not key off document.head — Chromium often creates an empty <head> before title/scripts.
   */
  function scheduleBoot() {
    ...
    // Enough distinctive DOM to decide this is not a blank document_start shell.
    if (document.title || document.getElementsByTagName('script').length || document.body) {
      boot();
      return;
    }

    let booted = false;
    const mo = new MutationObserver(function unlockCfEarlyObserve() {
      if (
    ...
    function tryBootOnce() {
      if (booted || window.__devtoolsUnlockInstalled) {
        return;
      }
      booted = true;
      try {
    ...
    // Do NOT install in queueMicrotask: it often runs before <title>/CF markers are parsed
    // and would hook the interstitial. Fallback only after a short delay.
    setTimeout(function unlockCfBootFallback() {
      if (!booted) {
        tryBootOnce();
      }
    }, 50);
  ...
  /** Install all anti-devtools hooks (idempotent via __devtoolsUnlockInstalled). */
  function installUnlock() {
    ...
    /** Whether CF challenge is active — navigation/reload must stay unblocked. */
    function allowCloudflareNavigation() {
    ...
    function installNavigationGuard() {
      ...
      locProto.reload = mimicNative(function unlockReload() {
        // CF Managed Challenge finishes via reload; never block during challenge.
        if (allowCloudflareNavigation()) {
          return saved.locationReload.apply(this, arguments);
        }
      ...
      window.history.back = mimicNative(function unlockBack() {
        if (allowCloudflareNavigation()) {
          return saved.historyBack.apply(this, arguments);
        }
      ...
      window.close = mimicNative(function unlockClose() {
        if (allowCloudflareNavigation()) {
          return saved.windowClose.apply(this, arguments);
        }
      ...
      function blockWrite(prop, desc) {
        ...
        Object.defineProperty(Element.prototype, prop, {
          ...
          set(value) {
            ...
            // CF challenge may rewrite root HTML during verification — do not interfere.
            if (allowCloudflareNavigation()) {
              return desc.set.call(this, value);
            }
